...
func _ready() -> void:
	...
	ServerConnection.send_packet(PacketUtils.Outgoing.REQUEST_LOBBY_SYNC)

func _on_packet_received(packet_id: int, data: PackedByteArray) -> void:
	match packet_id:
		PacketUtils.Incoming.LOBBY_SYNC:
			var lobby_info_res: Array = PacketUtils.read_lobby_sync(data)
			var lobby_info: Dictionary = lobby_info_res[PacketUtils.VALUE]

			lobby_name_label.text = lobby_info.lobby_name
			lobby_id_label.text = "ID: " + str(lobby_info.lobby_id)
			print(lobby_info.host_id)
		...
		PacketUtils.Incoming.SYNC_PLAYERS:
			...
			for player_data: Dictionary in sync_res[PacketUtils.VALUE]:
