...
func _ready() -> void:
	...
	ServerConnection.send_packet(PacketID.Outgoing.REQUEST_SYNC)

func _on_received_packet(packet_id: int, data: PackedByteArray) -> void:
	match packet_id:
		PacketID.Incoming.UPDATE_PLAYERS:
			_handle_player_update(ServerConnection.read_player_update(data, 0)[0])
		PacketID.Incoming.SYNC_PLAYERS:
			_handle_player_sync(ServerConnection.read_player_sync(data, 0)[0].players)
