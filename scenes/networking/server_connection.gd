...
func send_packet(packet_id: int, data: PackedByteArray = PackedByteArray()) -> void:
	...
	var packet_length: int = id_bytes.size() + data.size()
...
func _process_incoming_packets(available: int) -> void:
	...
	while _buffer_pos < _incoming_buffer.size():
		...
		if not length_res[PacketUtils.FULLY_READ]: break
		...
		if not packet_id_res[PacketUtils.FULLY_READ]: break
		...
		_buffer_pos = packet_id_res[1] + data_length
...
func _on_received_packet(packet_id: int, data: PackedByteArray) -> void:
	match packet_id:
		...
		PacketUtils.Incoming.PING:
			send_packet(PacketUtils.Outgoing.PONG)
		PacketUtils.Incoming.KICK_PLAYER:
			var kick_reason_res: Array = PacketUtils.read_string(data)
			disconnect_from_server("Kicked: %s"  % kick_reason_res[PacketUtils.VALUE])
